...
// import { LAB_ALIASES, LAB_CATALOG } from "./constants";
...
export const calcAlert = (
  ...
  return !Number.isNaN(value) && value >= 5;
};
...
export const mapBackendToUIName = (
  name: string
): ClinicalParameterName => {
  switch (name) {
    case "NEWS2":
 ...
 * Laboratory helpers
 * ============================================================ */
export const getLabMeta = (
  testName: string
): LabMeta | undefined => {
  if (LAB_CATALOG[testName]) {
    return LAB_CATALOG[testName];
  }

  const alias = LAB_ALIASES[testName];

  if (alias) {
    return LAB_CATALOG[alias];
  }
...
export const nowHHmm = () => {
  ...
  return `${pad2(date.getHours())}:${pad2(
    date.getMinutes()
  )}`;
};
...
export const formatBalanceMl = (ml: number) => {
  ...
  return `${sign}${Math.abs(ml).toLocaleString(
    "sv-SE"
  )} ml`;
};
...
export const formatMedicationDose = (item: any) => {
  const frequency = item.frequency?.replaceAll("_", " ");

  if (!frequency) {
    return item.dose;
  }

  const alreadyIncluded = item.dose
    ?.toLowerCase()
    .includes(frequency.toLowerCase());
...
export const prettifyReferralType = (
  value?: string
) => {
  if (!value) return "-";
